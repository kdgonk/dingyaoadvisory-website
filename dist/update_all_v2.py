#!/usr/bin/env python3
"""
全站英文頁面選單文字統一 + Logo 連結修正 (v2 - fixed)
"""
import os
import re
...
# ============================================================
# 2. Logo 加上 id="navLogo" - 修正版
# ============================================================
# 正確做法：在 class="nav-logo" 後面加上 id="navLogo"
# 匹配 <a href="..." class="nav-logo"> 並改成 <a href="..." class="nav-logo" id="navLogo">
...
def update_script(content):
    ...
    while i < len(lines):
        ...
        # Check if this line starts the old lang switcher script
        # Pattern: <script> followed by (function(){ var path = window.location.pathname; var tw = ...
        if '<script>' in line and i + 1 < len(lines):
            next_line = lines[i + 1].strip()
            if next_line.startswith('(function(){') and 'var path' in next_line:
                # Skip until we find </script>
                i += 1
                while i < len(lines):
