#!/usr/bin/env python3
"""
全站英文頁面選單文字統一 + Logo 連結修正 (v3 - fixed script detection)
"""
import os
import re
...
def replace_in_nav_links(content, is_english):
    ...
    # Find nav-links sections - match from <div class="nav-links"...> to the </div> that precedes <button class="nav-toggle"
...
def update_script(content):
    ...
    # Use regex to find the old script block
    # The old script starts with <script> then (function(){ then var path = window.location.pathname; etc.
    # Match from <script> through the closing })();</script>
    
    # Pattern: <script> followed by newline, then (function(){, then var path..., then var tw..., then var en..., 
    # then the rest of the script, then })(); then newline then </script>
    
    # More flexible: find <script> that contains the specific pattern
    ...
    result = re.sub(old_script_re, NEW_SCRIPT, content, flags=re.DOTALL)
