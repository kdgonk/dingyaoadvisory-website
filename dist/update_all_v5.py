#!/usr/bin/env python3
"""
全站英文頁面選單文字統一 + Logo 連結修正 (v5 - handles both navbar types)
"""
import os
import re
...
def replace_in_nav_links(content, is_english):
    """Replace menu text only inside nav-links divs (standard navbar)"""
    if not is_english:
        return content
...
def replace_in_tailwind_navbar(content, is_english):
    """Replace menu text in old Tailwind-style navbars (blog pages, project-durbanville)"""
    if not is_english:
        return content
    
    # These navbars are inside <nav class="max-w-7xl mx-auto bg-white/...">
    # They have links with classes like "px-4 py-2 text-sm font-medium text-gray-600..."
    # We need to find the text content of <a> tags within these navbars
    
    # Find the old Tailwind navbar sections
    # They start with <nav class="max-w-7xl mx-auto bg-white/...">
    # and end with </nav> followed by </div> then either <!-- NAVBAR --> or <section>
    
    # Strategy: find all <a> tags within the old Tailwind navbar and replace their text
    # The old navbar is between the first <nav class="max-w-7xl..."> and the next <!-- NAVBAR --> or <section>
    
    # Simpler approach: just do a global text replacement within the old navbar area
    # Find the old navbar: <nav class="max-w-7xl mx-auto bg-white/..."> ... </nav>
    pattern = r'(<nav class="max-w-7xl mx-auto bg-white[^>]*>)(.*?)(</nav>\s*</div>)'
...
def process_file(filepath, is_english):
    ...
    # Step 1: Replace menu text in standard nav-links (English only)
    content = replace_in_nav_links(content, is_english)
    
    # Step 2: Replace menu text in old Tailwind navbars (English only)
    content = replace_in_tailwind_navbar(content, is_english)
    
    # Step 3: Add id="navLogo" to logo anchor
